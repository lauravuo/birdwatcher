...
test.describe("Redirection Logic", () => {
	test("Group View -> Add Sighting redirects to User View", async ({
		...
		await authenticatedPage.locator(".bird-dropdown .bird-option").first().click();
		...
		await expect(authenticatedPage.locator(".add-sighting-dialog")).not.toBeVisible();
	});
	...
	test("Edit Sighting -> Redirects to/Stays on Sighting Details", async ({
		...
		await addSighting(authenticatedPage, "Varis", new Date().toISOString().split("T")[0]);
