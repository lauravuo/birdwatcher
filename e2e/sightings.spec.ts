...
test.describe("Sightings", () => {
	...
	test("add sighting button is visible", async ({ authenticatedPage, user }) => {
	...
	test("opens add sighting dialog when button is clicked", async ({ authenticatedPage, user }) => {
	...
	test("can add a sighting with required fields", async ({ authenticatedPage, user }) => {
		...
		// Dialog should close
		await expect(authenticatedPage.getByTestId("add-sighting-form")).not.toBeVisible({
			timeout: 5000,
		});
	});

	test("submit button is disabled when required fields are missing", async ({ authenticatedPage, user }) => {
	...
	test("can filter birds in autocomplete", async ({ authenticatedPage, user }) => {
	...
	test("can get current location", async ({ authenticatedPage, user, context }) => {
		...
		await expect(authenticatedPage.getByText("60.1699")).toBeVisible({ timeout: 5000 });
		await expect(authenticatedPage.getByText("24.9384")).toBeVisible();
	});

	test("sightings list shows added sightings", async ({ authenticatedPage, user }) => {
		...
		await expect(authenticatedPage.getByRole("heading", { name: /sightings/i })).toBeVisible({ timeout: 10000 });
		await expect(authenticatedPage.getByText(/no sightings found/i)).toBeVisible({ timeout: 5000 });
	});
});
