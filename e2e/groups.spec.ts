...
import { getGroupByCode, seedGroup, seedSightings } from "./helpers/firestore-helpers";
...
test.describe("Groups UI / Management", () => {
	...
	test("displays group management interface in dev mode", async ({ authenticatedPage }) => {
		// Wait for redirect to dashboard/groups
		await expect(authenticatedPage.getByText("Your Groups")).toBeVisible({ timeout: 10000 });
		await expect(authenticatedPage.getByText("You haven't joined any groups yet")).toBeVisible();

		// Check Create Form (Dev Only)
		await expect(
			authenticatedPage.getByRole("heading", { name: "Create New Group (Dev Only)" }),
		).toBeVisible();

		expect(logTypes).not.toContain("error");
	});

	test("successfully creates a group via UI and joins automatically", async ({ authenticatedPage, user }) => {
		...
		await authenticatedPage.getByRole("button", { name: "Create Group" }).click();

		// Verify owner sees the new group list
		const groupLink = authenticatedPage.getByRole("link", { name: new RegExp(groupName) });
		await expect(groupLink).toBeVisible({ timeout: 10000 });

		// Extract join code from UI using data-testid
		const joinCode = await groupLink.locator('[data-testid="join-code"]').textContent();
		expect(joinCode).toBeTruthy();
	});

	test("successfully joins group via URL", async ({ authenticatedPage, user }) => {
	...
	test("shows error for invalid join code", async ({ authenticatedPage }) => {
		...
		const errorMessage = authenticatedPage.getByText(/Failed to auto-join group/);
		await expect(errorMessage).toBeVisible({ timeout: 10000 });
	});

	test("shows member list when a group is selected", async ({ authenticatedPage, user }) => {
		...
		await seedGroup({ name: groupName, ownerId: user.uid, memberIds: [user.uid, "other-user"] });

		// Reload to fetch groups
		await authenticatedPage.reload();

		// Click the group in the list (Safe because no auto-redirect)
		const groupItem = authenticatedPage.getByRole("link", { name: new RegExp(groupName) });
		...
		await expect(authenticatedPage.locator(".member-name").filter({ hasText: user.displayName })).toBeVisible();
	});

	test("shows single group by default without back button", async ({ authenticatedPage, user }) => {
		...
		await expect(authenticatedPage.locator(".breadcrumbs").getByText(groupName)).toBeVisible({ timeout: 10000 });

		// Verify no manual back button
		await expect(authenticatedPage.getByRole("button", { name: "← Back" })).not.toBeVisible();
		await expect(authenticatedPage.getByText("Your Groups")).not.toBeVisible();
	});

	test("can toggle filters visibility in group view", async ({ authenticatedPage, user }) => {
	...
	test("owner can remove a member from the group", async ({ authenticatedPage, user }) => {
		const groupName = `Rm Group ${crypto.randomUUID().substring(0, 4)}`;
		const memberEmail = `member-${crypto.randomUUID()}@birdwatcher.test`;
		const memberUser = await createTestUser(memberEmail, "pass123", "RemovalMember");

		await seedGroup({ name: groupName, ownerId: user.uid, memberIds: [user.uid, memberUser.uid] });
		...
		const memberListItem = authenticatedPage.locator(".member-item").filter({ hasText: "RemovalMember" });
		const ownerListItem = authenticatedPage.locator(".member-item").filter({ hasText: user.displayName });

		await expect(memberListItem.locator("button.remove-member-button")).toBeVisible();
		await expect(ownerListItem.locator("button.remove-member-button")).not.toBeVisible();
	...
	test("toggles between month and year view in group", async ({ authenticatedPage, user }) => {
		const groupId = await seedGroup({ name: "Mode Group", memberIds: [user.uid] });
		...
		await expect(authenticatedPage.locator(".sightings-list").getByText("Harakka").first()).toBeVisible();
		if (month !== prevMonth) {
			await expect(authenticatedPage.locator(".sightings-list").getByText("Varis")).toBeHidden();
		}
		...
		if (prevYear === year) {
			await expect(authenticatedPage.locator(".sightings-list").getByText("Harakka").first()).toBeVisible();
			await expect(authenticatedPage.locator(".sightings-list").getByText("Varis").first()).toBeVisible();
		}

		await authenticatedPage.getByLabel("Month").selectOption(String(monthIndex));
		await expect(authenticatedPage.locator(".sightings-list").getByText("Harakka").first()).toBeVisible();
	});
});
